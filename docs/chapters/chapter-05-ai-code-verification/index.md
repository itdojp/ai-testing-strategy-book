...
> **この章の学習目標**
> - AI生成コードの検証観点（境界値/エッジケース/不変条件）を体系化できるようになること。
> - AI生成テストをレビューし、誤ったオラクル・過剰モック・フレーク要因を指摘できるようになること。
> - 失敗テストが「テスト不備」か「実装不備」かを切り分け、修正方針を立てられるようになること。
...
本章では、AI生成コードの品質を確保するための実践的な検証技法を詳しく探求した。主要な学びは以下のとおりである。
...
- [ ] レビュー観点は [付録B チェックリスト](../../appendices/appendix-b-checklists/) に沿って整理できている。

## ミニ演習（手を動かす）

- [ ] 自分のプロジェクトを想定し、「関数1つ」を選んで、AIにテスト雛形を生成させるためのプロンプトを作成する（正常系/異常系/境界値/不変条件を指定する）。
- [ ] 生成されたテストについて、[付録B チェックリスト](../../appendices/appendix-b-checklists/) を使って *最低3点* 指摘できるか確認する。
