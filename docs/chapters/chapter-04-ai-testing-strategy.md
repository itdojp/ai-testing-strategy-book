...
## 4.1 リスクベースアプローチの再定義
<a id="figure-ai-testing-lifecycle"></a>
<figure class="reader-figure">
  <img src="{{ '/assets/images/diagrams/ai-testing-lifecycle.svg' | relative_url }}" alt="計画、設計、実行、分析、最適化を循環させ、AI固有のデータ品質、モデル、バイアス、説明可能性、セキュリティを確認するテストライフサイクル図">
  <figcaption>図4-1 AIシステムのテストは、計画から最適化までを循環させ、AI固有の品質観点を各段階へ織り込む。</figcaption>
</figure>

**文章による代替**: テスト計画、設計、実行、結果分析、改善・最適化を繰り返す。全段階でデータ品質、モデル精度・再現率、バイアス、説明可能性、セキュリティとプライバシーを確認し、分析結果を次の計画へ戻す。
...
AI生成コードは継続的に進化し、品質も変動する。固定的な優先順位では、この変化に対応しきれない。リアルタイムのフィードバックに基づいて優先順位を調整する仕組みが必要である。
...
- [ ] テスト戦略を文書化できる（テンプレートは [付録A テンプレート集]({{ '/appendices/appendix-a-templates/' | relative_url }}) を参照）。

## ミニ演習（手を動かす）

- [ ] 自分のプロジェクトを想定し、「重要機能を1つ」選び、多次元リスクの観点（影響度/複雑度/AI理解度/データ機密性 等）で簡易スコアリングしてみる。
- [ ] その機能に対して、最小のテスト構成（ユニット/統合/異常系）を箇条書きで作成する。
- [ ] レビュー時の観点を [付録B チェックリスト]({{ '/appendices/appendix-b-checklists/' | relative_url }}) から3つ選び、PRレビューコメントの形に書いてみる。
