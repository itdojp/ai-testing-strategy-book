...
- テスト計画やテストレベルの切り分けを具体化する際は、[付録A テンプレート集](../appendices/appendix-a-templates/) に含まれるテスト計画書テンプレートが参考になる。
- テスト観点の抜け漏れを防ぎたい場合は、[付録B チェックリスト](../appendices/appendix-b-checklists/) を活用し、従来型テストと AI 特有の観点を統合したチェックリストを作成してほしい。
- 用語の意味に迷った場合は、[付録D 用語集](../appendices/appendix-d-glossary/) を参照し、本書全体での用語の使い方を確認するとよい。
