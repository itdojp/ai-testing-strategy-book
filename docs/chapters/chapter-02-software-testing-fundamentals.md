...
- **AI 活用の観点**：AIは賢いテストケース選択により、この問題を緩和できる。ペアワイズテスト、境界値分析の自動化など。
...
- [付録A テンプレート集]({{ '/appendices/appendix-a-templates/' | relative_url }})
...
<a id="figure-ai-testing-pyramid"></a>
<figure class="reader-figure">
  <img src="{{ '/assets/images/diagrams/ai-testing-pyramid.svg' | relative_url }}" alt="AIシステムのテストをユニット、統合、システム、E2Eの四層に分け、実行頻度とコストを示すテストピラミッド">
  <figcaption>図2-1 AIシステムのテストピラミッドは、低コストで頻繁に実行する下層を厚くし、上層で統合・利用者シナリオを確認する。</figcaption>
</figure>

**文章による代替**: ユニットテストを70%、統合テストを20%、システムテストを7%、E2Eテストを3%の目安として置く。上に行くほど実行コストは高く頻度は低くなる。各層ではデータ品質、モデル精度、バイアス、説明可能性、ドリフト、フェアネス、プライバシー、ロバスト性、再現性を横断して確認する。
...
- テストレベル、テストタイプ、テスト設計技法など、ソフトウェアテストの基本概念を体系的に整理し直した。
- AI主導開発であっても、品質モデルやリスクベーステストといった「テストの土台」は引き続き有効であることを確認した。
- 後続の章で扱う AI 特有の課題や戦略を理解するためには、従来のテスト基礎を前提としていることを明確にした。

### この章を読み終えたら確認したいこと

- [ ] 自分の言葉で、主要なテストレベル（単体／結合／システム／受入）と、その目的の違いを説明できるか。
- [ ] ブラックボックステスト／ホワイトボックステストなど、代表的なテスト技法の特徴と使い所を整理できているか。
- [ ] 自チームの現行テストプロセスが、本章で紹介したモデルや用語とどのように対応しているかをマッピングしてみたか。

### 関連する付録・テンプレート

- テスト計画やテストレベルの切り分けを具体化する際は、[付録A テンプレート集]({{ '/appendices/appendix-a-templates/' | relative_url }}) に含まれるテスト計画書テンプレートが参考になる。
- テスト観点の抜け漏れを防ぎたい場合は、[付録B チェックリスト]({{ '/appendices/appendix-b-checklists/' | relative_url }}) を活用し、従来型テストと AI 特有の観点を統合したチェックリストを作成してほしい。
- 用語の意味に迷った場合は、[付録D 用語集]({{ '/appendices/appendix-d-glossary/' | relative_url }}) を参照し、本書全体での用語の使い方を確認するとよい。
