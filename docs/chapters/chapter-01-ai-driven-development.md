...
近年、主要なAI支援開発ツールの導入は急速に進んでいる。導入状況は業界・職種・組織規模で差があるため、個別の割合を断定するのではなく、参考資料（例: [Stack Overflow Developer Survey 2024](https://survey.stackoverflow.co/2024/)）も確認しつつ、自組織の実態に合わせて捉える必要がある。

主要なAI支援は、用途で見ると次のカテゴリに分けられる（ツール名は変化しやすいため、カテゴリとして捉える）。
